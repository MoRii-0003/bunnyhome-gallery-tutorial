...
export class AnthropicProvider {
  ...
  async anthropicMessage(body) {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: this.config.anthropicModel, max_tokens: 1000, ...body }),
  ...
  async describeImageNeutral(image) {
    const result = await this.anthropicMessage({
      system: NEUTRAL_VISION_PROMPT,
      max_tokens: 400,
      temperature: 0.1,
      messages: [{
        role: 'user',
        content: [this.imageBlock(image), { type: 'text', text: 'Write the neutral private visual index now.' }],
      }],
    });
  ...
  async replyAsCompanion({ message, image, memory, requestMetadata = false }) {
    const content = [];
    if (image) content.push(this.imageBlock(image));
    if (memory) {
      content.push({
        type: 'text',
        text: [
    ...
    const result = await this.anthropicMessage({
      system: this.config.companionSystemPrompt,
